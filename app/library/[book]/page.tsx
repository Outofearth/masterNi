    ...
    <div style={{ background: 'var(--bg-page)', minHeight: '100vh' }}>
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="text-center mb-12">
          <div style={{ fontSize: '13px', color: 'var(--tx-3)', letterSpacing: '0.3em', marginBottom: '8px' }}>
          ...
          <p style={{ fontSize: '15px', color: 'var(--tx-2)', lineHeight: 1.8, maxWidth: '500px', margin: '0 auto' }}>
            {book.intro}
          </p>
        </div>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: '14px', border: '1px solid rgba(184,146,42,0.2)', overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid rgba(184,146,42,0.15)', fontSize: '13px', color: 'var(--tx-3)', letterSpacing: '0.3em' }}>
            CHAPTERS · 章节目录
          </div>
          {book.chapters.map((chapter, i) => (
            ...
            >
              <div style={{ fontSize: '14px', color: 'var(--ac-text)', fontWeight: 600, minWidth: '40px', letterSpacing: '0.1em' }}>
                {String(i + 1).padStart(2, '0')}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '16px', color: 'var(--tx-0)', fontWeight: 500, letterSpacing: '0.08em', marginBottom: '2px' }}>
                  {chapter.title}
                </div>
                {chapter.subtitle && (
              ...
              <div style={{ fontSize: '12px', color: 'var(--tx-3)', letterSpacing: '0.1em' }}>
                {chapter.paragraphs.length} 段
              </div>
              <div style={{ fontSize: '14px', color: 'var(--ac-text)' }}>→</div>
            </Link>
          ))}
        </div>
      <SiteFooter />
