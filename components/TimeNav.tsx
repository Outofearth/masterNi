      ...
      >
        ...
        >
          <button
            ...
            className="text-[12px] font-medium flex-1 text-center"
          ...
          <div className="flex items-center gap-0.5">
            <button
              ...
              className="text-[11px] w-4 h-4 flex items-center justify-center rounded"
              style={{ color: 'var(--t-faint)' }}
            >
              ‹
            </button>
            <span
              className="text-[12px] font-mono min-w-[28px] text-center cursor-pointer"
              style={{ color: view === 'liunian' ? 'var(--t-gold)' : 'var(--t-faint)' }}
              onClick={() => onViewChange('liunian')}
            >
              {liunianYear}
      ...
      {overlayInfo && (
        ...
        >
          ...
          {(['禄', '权', '科', '忌'] as const).map(sh => {
            ...
            return (
              <span key={sh} className="text-[11px] font-medium" style={{ color: SIHUA_COLORS[sh] }}>
                {starName}化{sh}
              </span>
            );
          })}
