  ...
  return (
    <div className="space-y-0.5">
      {lines.map((line, i) => {
        ...
        return (
          <div key={i} className="text-[15px] leading-relaxed" style={{ color: 'var(--t-text2)' }}>
            {parts.map((part, j) =>
              j % 2 === 0
                ? part
...
function StarQuickLinks({ links }: { links: QuickLink[] }) {
  return (
    ...
    >
      <div className="text-[12px] tracking-widest mb-2 flex items-center gap-1.5" style={{ color: 'var(--t-faint)' }}>
        ...
        {links.map(l => (
          <Fragment key={`${l.palace}-${l.star}`}>
            <Link
              ...
              className="text-[13px] px-2 py-1 rounded-md no-underline transition-opacity hover:opacity-80"
              style={{
...
基于此星此宫配置的具体可操作建议。`;
  ...
  /** 导出：排盘 + 当前解读/对话 → PDF 报告 */
  const handleExport = () => {
    exportChartPdf(chart, messagesRef.current, '紫微斗数 · AI 解盘报告');
  };
          ...
          {TOPICS.map(t => {
            ...
            return (
              <button
                ...
                disabled={loading}
                className="py-1.5 text-[13px] font-medium rounded-lg transition-all duration-150 disabled:opacity-40"
                style={{
          ...
          {messages.map((msg, i) => {
            ...
            if (msg.role === 'user') {
              return (
                ...
                >
                  <div
                    className="max-w-[85%] rounded-xl px-3.5 py-2 text-[15px] leading-relaxed"
                    style={{
            ...
            return (
              ...
              >
                <div
                  className="text-[12px] tracking-widest mb-2 flex items-center gap-1.5"
      ...
      <div className="flex-shrink-0 px-3 pb-3 pt-2" style={{ borderTop: '1px solid var(--t-border)' }}>
        <div className="flex gap-2">
          <input
            ...
            disabled={loading}
            className="flex-1 rounded-lg px-3.5 py-2 text-[15px] focus:outline-none transition-colors"
            style={{
              background: 'var(--t-card)',
              border: '1px solid var(--t-border)',
              color: 'var(--t-text)',
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-lg text-[15px] font-medium transition-all disabled:opacity-30 disabled:cursor-not-allowed"
            style={{
