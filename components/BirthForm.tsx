...
export default function BirthForm({ onSubmit, loading, initialData, onFormSave, hideSubmit }: BirthFormProps) {
  ...
  // ── 原生下拉（select）专用样式 ──
  // 暗色模式用「不透明深底 + 浅色高对比文字」：原生 <select> 展开的选项列表会沿用该底色，
  // 若仍用半透明浅色底会出现「白底 + 浅字 / 深底 + 深字」的难读情况。
  // option 弹出列表的兜底样式见 app/globals.css 的 select option 规则。
  const selectBg = isDark ? '#131c36' : 'rgba(255,252,240,0.95)';
  const selectStyle = { ...inputStyle, background: selectBg };
  const selectErrorStyle = { ...selectStyle, borderColor: errorClr };
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
          <div>
            <select
              ...
              style={showErr('year') && errors.year ? selectErrorStyle : selectStyle}
              required
            >
          ...
          <div>
            <select
              ...
              style={showErr('month') && errors.month ? selectErrorStyle : selectStyle}
              required
            >
          ...
          <div>
            <select
              ...
              style={showErr('day') && errors.day ? selectErrorStyle : selectStyle}
              required
            >
